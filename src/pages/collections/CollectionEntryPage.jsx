...
import { mockCollections, mockDealers, mockSalespersons } from '../../mock';
import { formatDate, formatCurrency, todayISO } from '../../utils';
import { PAYMENT_MODES } from '../../constants';
import { useToast } from '../../hooks/useToast';

const EMPTY_FORM = { date: todayISO(), dealerId: '', salespersonId: '', amount: '', mode: 'Cash', reference: '' };
...
export default function CollectionEntryPage() {
  ...
  const handleSave = (addNew = false) => {
    ...
    const row = {
      ...
      salesperson: sp?.name, amount: Number(form.amount), mode: form.mode,
      reference: form.reference, status: 'Pending',
    };
  ...
  const columns = [
    ...
    { header: 'Mode', key: 'mode', render: (r) => (
      <span className="text-xs px-1.5 py-0.5 bg-gray-100 rounded font-medium">{r.mode}</span>
    )},
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Record daily cash and credit collections from dealers"
      ...
      <DataTable
        columns={columns}
        data={collections}
        searchKeys={['id', 'dealer', 'salesperson', 'mode', 'reference', 'status']}
      ...
      >
        <div className="grid grid-cols-2 gap-4">
          <SelectInput label="Dealer" id="dealer" required value={form.dealerId} onChange={(e) => setField('dealerId', e.target.value)} error={errors.dealerId} placeholder="Select dealer">
            {mockDealers.filter((d) => d.status === 'Active').map((d) => <option key={d.id} value={d.id}>{d.name} {d.outstanding > 0 ? `(₹${d.outstanding} outstanding)` : ''}</option>)}
          </SelectInput>
