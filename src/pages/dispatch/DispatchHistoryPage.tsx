...
export default function DispatchHistoryPage() {
  const columns: TableColumn<Dispatch>[] = [
    ...
    { header: 'Items',       key: 'items',            render: (r) => <span className="text-gray-600">{r.items.length} product{r.items.length > 1 ? 's' : ''}</span> },
    { header: 'Total Qty',   accessor: 'totalQty',    sortable: true, key: 'qty', render: (r) => <span className="font-semibold tabular-nums">{formatNumber(r.totalQty)}</span> },
