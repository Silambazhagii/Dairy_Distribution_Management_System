...
import { mockSales, mockDealers, mockRoutes, mockSalespersons } from '../../mock';
import { formatDate, formatCurrency, todayISO } from '../../utils';
import { SALE_TYPES } from '../../constants';
import { useToast } from '../../hooks/useToast';

const EMPTY_FORM = {
  date: todayISO(), dealerId: '', salespersonId: '', soldQty: '',
...
export default function SalesEntryPage() {
  ...
  const validate = () => {
    const e = {};
    if (!form.dealerId) e.dealerId = 'Select dealer';
  ...
  const handleSave = (addNew = false) => {
    ...
    const dealer = mockDealers.find((d) => d.id === form.dealerId);
    const sp = mockSalespersons.find((s) => s.id === form.salespersonId);
    const sold = Number(form.soldQty);
    const price = Number(form.unitPrice);
    const discount = Number(form.discount) || 0;
    const amount = sold * price - discount;
    const row = {
      id: `INV-2024-${String(sales.length + 455).padStart(4, '0')}`,
      date: form.date, dealer: dealer?.name, route: dealer?.route,
      salesperson: sp?.name, soldQty: sold, freeQty: Number(form.freeQty) || 0,
      returnQty: Number(form.returnQty) || 0, amount, discount, saleType: form.saleType,
      status: 'Confirmed',
    };
    setSales((s) => [row, ...s]);
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Record daily sales by dealer and salesperson"
      ...
      <DataTable
        columns={columns}
        data={sales}
        searchKeys={['id', 'dealer', 'salesperson', 'route', 'saleType', 'status']}
      ...
      >
        <div className="grid grid-cols-2 gap-4">
          <SelectInput label="Dealer" id="dealer" required value={form.dealerId} onChange={(e) => setField('dealerId', e.target.value)} error={errors.dealerId} placeholder="Select dealer">
            {mockDealers.filter((d) => d.status === 'Active').map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}
          </SelectInput>
      ...
      <Modal open={!!viewItem} onClose={() => setViewItem(null)} title="Invoice Details" size="sm">
        {viewItem && (
          <dl className="space-y-2">
            {[
              ['Invoice', viewItem.id], ['Date', formatDate(viewItem.date)], ['Dealer', viewItem.dealer],
              ['Salesperson', viewItem.salesperson], ['Sold Qty', viewItem.soldQty],
              ['Free Qty', viewItem.freeQty], ['Return Qty', viewItem.returnQty],
              ['Amount', formatCurrency(viewItem.amount)], ['Discount', formatCurrency(viewItem.discount)],
              ['Sale Type', viewItem.saleType], ['Status', <StatusBadge key="st" status={viewItem.status} />],
            ].map(([k, v]) => (
