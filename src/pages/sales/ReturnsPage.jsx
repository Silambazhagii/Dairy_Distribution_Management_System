...
const RETURN_REASONS = ['Expired', 'Damaged', 'Quality issue', 'Wrong product', 'Excess delivery', 'Other'];

const mockReturns = [
  { id: 'RET-001', date: '2026-08-06', invoiceId: 'INV-2024-0451', dealer: 'Kumaran Dairy', qty: 10, reason: 'Quality issue', status: 'Verified', action: 'Spoiled' },
  { id: 'RET-002', date: '2026-08-05', invoiceId: 'INV-2024-0454', dealer: 'Kavitha Mart', qty: 2, reason: 'Damaged', status: 'Pending', action: 'Pending' },
];
...
export default function ReturnsPage() {
  ...
  const [form, setForm] = useState({ invoiceId: '', qty: '', reason: '', notes: '' });
  ...
  const handleSave = () => {
    ...
    const row = {
      ...
      date: todayISO(), invoiceId: form.invoiceId,
      dealer: invoice?.dealer || '', qty: Number(form.qty),
      reason: form.reason, status: 'Pending', action: 'Pending',
    };
    setReturns((r) => [row, ...r]);
    toast('Return recorded successfully', 'success');
    setModalOpen(false);
    setForm({ invoiceId: '', qty: '', reason: '', notes: '' });
  };
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<button onClick={() => { setForm({ invoiceId: '', qty: '', reason: '', notes: '' }); setErrors({}); setModalOpen(true); }} className="btn-primary gap-1.5"><Plus className="w-4 h-4" /> Record Return</button>}
      />

      <DataTable
        columns={columns}
        data={returns}
        searchKeys={['id', 'invoiceId', 'dealer', 'reason', 'status']}
      ...
      >
        <div className="space-y-4">
          <SelectInput label="Invoice" id="invoice" required value={form.invoiceId} onChange={(e) => setField('invoiceId', e.target.value)} error={errors.invoiceId} placeholder="Select invoice">
            {mockSales.map((s) => <option key={s.id} value={s.id}>{s.id} – {s.dealer}</option>)}
          </SelectInput>
