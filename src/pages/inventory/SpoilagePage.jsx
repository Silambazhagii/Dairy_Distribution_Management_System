...
const EMPTY_FORM = { batchId: '', qty: '', reason: '' };
const REASONS = ['Leaking packets', 'Damaged in transit', 'Expired', 'Torn packaging', 'Cold chain failure', 'Other'];

export default function SpoilagePage() {
  const toast = useToast();
  const [records, setRecords] = useState(mockSpoilage);
  ...
  const handleApprove = () => {
    setRecords((r) => r.map((x) => x.id === approveItem.id ? { ...x, status: 'Verified' } : x));
