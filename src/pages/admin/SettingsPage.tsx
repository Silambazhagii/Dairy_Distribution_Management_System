...
interface OperationalForm {
  ...
  creditWarningDays: string;
}

export default function SettingsPage() {
  const toast = useToast();
  const [companyForm, setCompanyForm] = useState<CompanyForm>({
    name: 'SriLakshmi Dairy',
    address: '123 Dairy Road, Chennai',
    phone: '9876543200',
    email: 'info@srilakshmidairy.com',
    gstin: '33XXXXX1234X1Z5'
  });
  const [opForm, setOpForm]           = useState<OperationalForm>({
  ...
  const handleSaveCompany = (e: React.FormEvent<HTMLFormElement>) => { e.preventDefault(); toast('Company settings saved', 'success'); };
  const handleSaveOp      = (e: React.FormEvent<HTMLFormElement>) => { e.preventDefault(); toast('Operational settings saved', 'success'); };

  const setComp = (k: keyof CompanyForm, v: string) => setCompanyForm((f) => ({ ...f, [k]: v }));
  const setOp   = (k: keyof OperationalForm, v: string) => setOpForm((f) => ({ ...f, [k]: v }));
