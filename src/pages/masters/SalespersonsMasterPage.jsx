...
import { mockSalespersons, mockRoutes } from '../../mock';
import { STATUS } from '../../constants';
import { formatDate } from '../../utils';
import { useToast } from '../../hooks/useToast';

const EMPTY_FORM = { name: '', phone: '', joinDate: '', status: STATUS.ACTIVE };
...
export default function SalespersonsMasterPage() {
  ...
  const handleSave = () => {
    ...
    if (editItem) {
      setSalespersons((s) => s.map((x) => x.id === editItem.id ? { ...x, ...form } : x));
      toast('Salesperson updated', 'success');
    } else {
      setSalespersons((s) => [...s, { ...form, id: `SP${String(salespersons.length + 7).padStart(3, '0')}`, routes: [] }]);
  ...
  const toggleStatus = (row) => {
    setSalespersons((s) => s.map((x) => x.id === row.id ? { ...x, status: x.status === STATUS.ACTIVE ? STATUS.INACTIVE : STATUS.ACTIVE } : x));
    toast(`Salesperson ${row.status === STATUS.ACTIVE ? 'deactivated' : 'activated'}`, 'info');
  };

  const openEdit = (row) => { setEditItem(row); setForm({ name: row.name, phone: row.phone, joinDate: row.joinDate, status: row.status }); setErrors({}); setModalOpen(true); };
  ...
  return (
    <div>
      ...
      />
      <DataTable columns={columns} data={salespersons} searchKeys={['id', 'name', 'phone', 'status']} searchPlaceholder="Search salespersons…" exportFilename="salespersons"
        actions={(row) => (
          <>
