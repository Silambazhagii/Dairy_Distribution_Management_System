...
import type { Product, GeneralStatus } from '../../types';
...
const toTableData = (items: Product[]): ProductWithStatus[] => items.map((p) => ({ ...p, status: STATUS.ACTIVE as GeneralStatus }));

export default function ProductsMasterPage() {
  const toast = useToast();
  const [products, setProducts]   = useState<ProductWithStatus[]>(toTableData(PRODUCTS));
  const [modalOpen, setModalOpen] = useState(false);
  const [editItem, setEditItem]   = useState<ProductWithStatus | null>(null);
  const [form, setForm]           = useState<ProductForm>({ category: '', name: '', packSize: '', unit: 'pcs', price: '' });
  const [errors, setErrors]       = useState<FormErrors>({});

  const setField = (k: keyof ProductForm, v: string) => { setForm((f) => ({ ...f, [k]: v })); setErrors((e) => ({ ...e, [k]: '' })); };
  ...
  const handleSave = () => {
    ...
    if (editItem) {
      setProducts((p) => p.map((x) => x.id === editItem.id ? { ...x, ...form, category: form.category as Product['category'] } : x));
      ...
      setProducts((p) => [...p, {
        ...
        id: `custom-${Date.now()}`,
        status: STATUS.ACTIVE as GeneralStatus
      }]);
  ...
  const toggleStatus = (row: ProductWithStatus) => {
    setProducts((p) => p.map((x) => x.id === row.id ? { ...x, status: (x.status === STATUS.ACTIVE ? STATUS.INACTIVE : STATUS.ACTIVE) as GeneralStatus } : x));
