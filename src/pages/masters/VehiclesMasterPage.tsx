...
import type { Vehicle, GeneralStatus } from '../../types';
import type { TableColumn } from '../../types';

const TYPES = ['Mini Truck', 'Auto', 'Cycle', 'Bike', 'Van'] as const;
...
const EMPTY_FORM: VehicleForm = { regNumber: '', type: '', capacity: '', assignedTo: '', status: STATUS.ACTIVE };
...
export default function VehiclesMasterPage() {
  ...
  const setField = (k: keyof VehicleForm, v: string) => { setForm((f) => ({ ...f, [k]: v })); setErrors((e) => ({ ...e, [k]: '' })); };
  ...
  const handleSave = () => {
    ...
    if (editItem) {
      setVehicles((v) => v.map((x) => x.id === editItem.id ? {
        ...x,
  ...
  const toggleStatus = (row: Vehicle) => {
    setVehicles((v) => v.map((x) => x.id === row.id ? { ...x, status: (x.status === STATUS.ACTIVE ? STATUS.INACTIVE : STATUS.ACTIVE) as GeneralStatus } : x));
  };

  const openEdit = (row: Vehicle) => { setEditItem(row); setForm({ regNumber: row.regNumber, type: row.type, capacity: String(row.capacity), assignedTo: row.assignedTo || '', status: row.status }); setErrors({}); setModalOpen(true); };
  ...
  return (
    <div>
      ...
      />
      <DataTable<Vehicle> columns={columns} data={vehicles} searchKeys={['id', 'regNumber', 'type', 'status']} searchPlaceholder="Search vehicles…" exportFilename="vehicles"
        actions={(row) => (
          <>
