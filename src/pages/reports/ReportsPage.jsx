...
import { BarChart2, FileText, TrendingUp, Package, Wallet, AlertTriangle } from 'lucide-react';
import PageHeader from '../../components/ui/PageHeader';
import { DateInput, SelectInput } from '../../components/ui/FormInputs';
import { mockSales, mockCollections, mockBatches, mockSpoilage } from '../../mock';
import { formatCurrency, formatDate, formatNumber, todayISO } from '../../utils';
import {
...
const REPORT_TYPES = [
  ...
  { id: 'collections', label: 'Collections Report', icon: Wallet, color: 'green' },
  { id: 'stock', label: 'Stock Report', icon: Package, color: 'purple' },
  { id: 'spoilage', label: 'Spoilage Report', icon: AlertTriangle, color: 'red' },
];
...
export default function ReportsPage() {
  ...
  const colorMap = { blue: 'bg-blue-50 text-blue-700 border-blue-200', green: 'bg-green-50 text-green-700 border-green-200', purple: 'bg-purple-50 text-purple-700 border-purple-200', red: 'bg-red-50 text-red-700 border-red-200' };
          ...
          <div className="card">
            ...
            <div className="card-body pt-0">
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={salesData}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" /><XAxis dataKey="name" tick={{ fontSize: 11 }} /><YAxis tick={{ fontSize: 11 }} tickFormatter={(v) => `₹${v / 1000}k`} /><Tooltip formatter={(v) => formatCurrency(v)} /><Legend wrapperStyle={{ fontSize: 11 }} /><Bar dataKey="cash" name="Cash" fill="#22c55e" radius={[3, 3, 0, 0]} /><Bar dataKey="credit" name="Credit" fill="#3b82f6" radius={[3, 3, 0, 0]} /></BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        </div>
      ...
      {activeReport === 'collections' && (
        <div className="space-y-4">
          ...
          <div className="card">
            ...
            <div className="card-body pt-0">
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={collectionData}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" /><XAxis dataKey="name" tick={{ fontSize: 11 }} /><YAxis tick={{ fontSize: 11 }} tickFormatter={(v) => `₹${v / 1000}k`} /><Tooltip formatter={(v) => formatCurrency(v)} /><Bar dataKey="amount" name="Amount" fill="#22c55e" radius={[3, 3, 0, 0]} /></BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="card-body pt-0">
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={stockData} layout="vertical"><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" horizontal={false} /><XAxis type="number" tick={{ fontSize: 11 }} /><YAxis dataKey="name" type="category" tick={{ fontSize: 11 }} width={90} /><Tooltip /><Legend wrapperStyle={{ fontSize: 11 }} /><Bar dataKey="received" name="Received" fill="#dbeafe" radius={[0, 3, 3, 0]} /><Bar dataKey="available" name="Available" fill="#3b82f6" radius={[0, 3, 3, 0]} /></BarChart>
            </ResponsiveContainer>
          </div>
        </div>
      )}
          ...
          <div className="card">
            ...
            <div className="card-body pt-0">
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={spoilageData}><CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" /><XAxis dataKey="name" tick={{ fontSize: 11 }} /><YAxis tick={{ fontSize: 11 }} /><Tooltip /><Bar dataKey="qty" name="Units Spoiled" fill="#ef4444" radius={[3, 3, 0, 0]} /></BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        </div>
