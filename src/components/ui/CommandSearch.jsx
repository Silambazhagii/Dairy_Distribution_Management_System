...
import { mockDealers, mockSalespersons, mockBatches, mockSales } from '../../mock';
...
const NAV_ITEMS = [
  ...
  { id: 'ledger',        label: 'Dealer Ledger',           path: '/collections/ledger',        icon: BookOpen,        group: 'Collections' },
...
export default function CommandSearch({ open, onClose }) {
  ...
  const dynamicItems = useMemo(() => [
    ...mockDealers.slice(0, 20).map((d) => ({
      id: `dealer-${d.id}`, label: d.name, subtitle: `Dealer · ${d.route}`,
      path: '/collections/ledger', icon: BookOpen, group: 'Dealers',
    })),
    ...mockSalespersons.slice(0, 10).map((s) => ({
      id: `sp-${s.id}`, label: s.name, subtitle: `Salesperson`,
    ...
    ...mockSales.slice(0, 10).map((s) => ({
      id: `inv-${s.id}`, label: s.id, subtitle: `Invoice · ${s.dealer}`,
  ...
  const displayLabel = query.trim() ? null : 'Recent';
        ...
        <div className="mx-4 bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100">
            ...
            <input
              ...
              placeholder="Search pages, dealers, invoices, batches…"
